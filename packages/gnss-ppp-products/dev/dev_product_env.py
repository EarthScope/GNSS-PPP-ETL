...
class WorkSpace:
    ...
    def add_resource_spec(self,path:Path|str,id: Optional[str] = None) -> None:
        ...
        if id is not None:
            name = id
        assert name not in self._resource_specs, f"Resource spec with name '{name}' already exists. Please choose a unique name."
        self._resource_specs[name] = spec

    def register_spec(
            self,base_dir: Path | str,spec_ids: List[str],alias: Optional[str] = None   ) -> None:
        ...
        for spec_id in spec_ids:
            assert spec_id in self._resource_specs, f"Spec id '{spec_id}' not found. Available specs: {list(self._resource_specs.keys())}"
...
if __name__ == "__main__":
    ...
    for path in Path(LOCAL_SPEC_DIR).glob("*.yaml"):
        workspace.add_resource_spec(path,id="default")
    base_dir = Path("/Volumes/DunbarSSD/Project/SeafloorGeodesy/GNSS-PPP")
    workspace.register_spec(base_dir=base_dir,spec_ids=["default"],alias="default")
